...
//         const { data } = await axios.get(`${baseUrl}rejctedCard`, {
//           headers: {
...
//       <h2 className="text-2xl font-semibold my-4">Rejected Card Requests</h2>
...
const RejectedCards = () => {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = cardRequests.filter((card) =>
      ["name", "mobile", "email"].some((key) =>
          ...
          <tbody>
            {filteredCards.map((card) => (
              <tr key={card._id} className="hover:bg-gray-100">
                <td className="py-2 px-4 border-b">{card.name}</td>
                <td className="py-2 px-4 border-b">{card.mobile}</td>
                <td className="py-2 px-4 border-b">{card.email || "N/A"}</td>
                <td className="py-2 px-4 border-b">
                  <button
                    onClick={() => handleCardSelect(card)}
                    className="text-blue-600 hover:underline"
