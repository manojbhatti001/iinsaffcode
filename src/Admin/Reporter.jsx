...
const Reporter = () => {
  const [advertisers, setAdvertisers] = useState([]); // State to hold reporters
  ...
  useEffect(() => {
    const fetchAdvertisers = async () => {
      try {
        ...
        setAdvertisers(response.data); // Set the state with the fetched reporters
      } catch (err) {
    ...
    fetchAdvertisers(); // Call the function to fetch reporters
  }, []); // Empty dependency array to run once on mount

  const toggleViewDetails = (id) => {
    // Toggle the visibility of details for a specific reporter
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4 text-center">Reporters</h1>
